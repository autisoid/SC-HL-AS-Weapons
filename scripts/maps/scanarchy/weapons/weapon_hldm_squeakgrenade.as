...
string g_PeeMdl		= "models/hlclassic/p_squeak.mdl";
string g_VeeMdl		= "models/hlclassic/v_squeak.mdl";
string g_WeeMdl		= "models/hlclassic/w_squeak.mdl";
...
class CSqueakGrenade : ScriptBaseMonsterEntity
{
	...
	void Spawn( void )
    {
        ...
        @m_pDmgPop = g_EngineFuncs.CVarGetPointer("sk_snark_dmg_pop");
	...
	void Killed( entvars_t@ pevAttacker, int iGib )
    {
        ...
        // play squeek blast
        g_SoundSystem.EmitSoundDyn(self.edict(), CHAN_ITEM, "squeek/sqk_blast1.wav", 1, 0.5, 0, PITCH_NORM);	

        CSoundEnt@ pSoundEnt = GetSoundEntInstance();
        pSoundEnt.InsertSound ( bits_SOUND_COMBAT, pev.origin, SMALL_EXPLOSION_VOLUME, 3.0, self );

        g_Utility.BloodDrips( pev.origin, g_vecZero, BloodColor(), 80 );

        if (m_hOwner.IsValid())
            g_WeaponFuncs.RadiusDamage ( self.pev.origin, pev, m_hOwner.GetEntity().pev, pev.dmg, pev.dmg * 2.5, CLASS_NONE, DMG_BLAST );
        else
            g_WeaponFuncs.RadiusDamage ( self.pev.origin, pev, pev, pev.dmg, pev.dmg * 2.5, CLASS_NONE, DMG_BLAST );
...
class CSqueak : ScriptBasePlayerWeaponEntity
{
    ...
    {
        Precache( );
        g_EntityFuncs.SetModel(self, self.GetW_Model("models/hlclassic/w_sqknest.mdl"));
    ...
    void PrimaryAttack()
    {
        if (m_pPlayer.m_rgAmmo(self.m_iPrimaryAmmoType) != 0)
        {
            ...
            if (tr.fAllSolid == 0 && tr.fStartSolid == 0 && tr.flFraction > 0.25)
            {
                ...
                CBaseEntity@ pSqueak = g_EntityFuncs.Create( "monster_sca_snark", tr.vecEndPos, m_pPlayer.pev.v_angle, false, m_pPlayer.edict() );
...
void Register()
{
    if (g_eModelsMode == kOpposingForce) {
        g_VeeMdl = "models/scanarchy/opfor/v_squeak.mdl";
    } else if (g_eModelsMode == kBlueShift) {
        g_VeeMdl = "models/scanarchy/bshift/v_squeak.mdl";
    }

    g_CustomEntityFuncs.RegisterCustomEntity("HLDMSQUEAK::CSqueakGrenade", "monster_sca_snark");
    g_CustomEntityFuncs.RegisterCustomEntity("HLDMSQUEAK::CSqueak", "weapon_hldm_squeakgrenade");
    g_ItemRegistry.RegisterWeapon("weapon_hldm_squeakgrenade", "scanarchy", "Snarks", "", "weapon_hldm_squeakgrenade", "");
}
