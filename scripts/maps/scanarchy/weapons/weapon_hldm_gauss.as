...
// Models
string g_PeeMdl               = "models/hlclassic/p_gauss.mdl";
string g_VeeMdl               = "models/hlclassic/v_gauss.mdl";
string g_WeeMdl               = "models/hlclassic/w_gauss.mdl";

// Sounds
const string g_Disch1Snd            = "hlclassic/weapons/electro4.wav";
const string g_Disch2Snd            = "hlclassic/weapons/electro5.wav";
const string g_Disch3Snd            = "hlclassic/weapons/electro6.wav";
const string g_SpinSnd              = "ambience/pulsemachine.wav";
const string g_ShootSnd             = "hlclassic/weapons/gauss2.wav";
const string g_EmptySnd             = "hlclassic/weapons/357_cock1.wav"; //NO!
...
const string g_WeaponName           = "weapon_hldm_gauss";
...
class weapon_hldm_gauss : ScriptBasePlayerWeaponEntity
{
    ...
    void Spawn()
    {
        ...
        @m_pDamage = g_EngineFuncs.CVarGetPointer( "sk_plr_gauss" ); // 20 vanilla HL, 19 SC(???);
    }
    ...
    bool Deploy()
    {
        ...
        return self.DefaultDeploy( self.GetV_Model(g_VeeMdl), self.GetP_Model(g_PeeMdl), GAUSS_DRAW, "gauss" );
    }
    ...
    void SecondaryAttack()
    {
        ...
        else if( m_iInAttack == iChargingStart )
        {
            ...
            g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, g_SpinSnd, 1.0f, ATTN_NORM, m_iSoundState, 110 );
        }
        else
        {
            ...
            if( g_Engine.time >= m_flNextAmmoBurn && m_flNextAmmoBurn != 1000 )
            {
                ...
                m_flNextAmmoBurn = g_Engine.time + 0.3f; // 0.1 for HLDM, 0.3 for SC
            }
    ...
    void StartFire()
    {
        ...
        {
            flDamage = m_pDamage.value * 10; //200
        }
        else
        {
            flDamage = ( m_pDamage.value * 10 ) * ( ( g_Engine.time - m_flStartCharge ) / GetFullChargeTime() ); //200
        }
    ...
    void Fire( Vector vecOrigSrc, Vector vecDir, float flDamage )
    {
        ...
        g_SoundSystem.PlaySound( m_pPlayer.edict(), CHAN_WEAPON, g_ShootSnd, 0.5f + flDamage * ( 0.40 / 400.0f ), ATTN_NORM, 0 );
                    ...
                    if( !m_bPrimaryFire )
                    {
                        ...
                        if( beam_tr.fAllSolid == 0 )
                        {
                            ...
                            if( n < flDamage )
                            {
                                ...
                                float damage_radius = flDamage * ( 1.75f);
...
void Register()
{
    if (g_eModelsMode == kOpposingForce) {
        g_VeeMdl = "models/scanarchy/opfor/v_gauss.mdl";
    }
    
    g_CustomEntityFuncs.RegisterCustomEntity( "HLDMGAUSS::weapon_hldm_gauss", g_WeaponName );
    g_ItemRegistry.RegisterWeapon( g_WeaponName, "scanarchy", g_PriAmmoType, "", "ammo_gaussclip", "");
}
