...
string g_VeeMdl			= "models/hlclassic/v_crowbar.mdl"; // Tayklor fix <3
string g_WeeMdl			= "models/hlclassic/w_crowbar.mdl";
string g_PeeMdl			= "models/hlclassic/p_crowbar.mdl";

const string g_Hit1Snd			= "hlclassic/weapons/cbar_hit1.wav";
const string g_Hit2Snd			= "hlclassic/weapons/cbar_hit2.wav";
const string g_Miss1Snd			= "hlclassic/weapons/cbar_miss1.wav";
const string g_HitBod1Snd		= "hlclassic/weapons/cbar_hitbod1.wav";
const string g_HitBod2Snd		= "hlclassic/weapons/cbar_hitbod2.wav";
const string g_HitBod3Snd		= "hlclassic/weapons/cbar_hitbod3.wav";
...
const string g_WeaponName		= "weapon_hldm_crowbar";
...
class weapon_clcrowbar : ScriptBasePlayerWeaponEntity
{
    ...
    void Touch(CBaseEntity@ _Other) {
        ...
        if (m_flLastBounceSoundTime + 0.3f < g_Engine.time) {
            if (self.pev.velocity != g_vecZero) {
                ...
                if (flDistance < 1.f) {
                    g_SoundSystem.PlaySound(self.edict(), CHAN_VOICE, "debris/metal2.wav", 0.95, ATTN_NORM, 0, PITCH_HIGH, 1, true, self.pev.origin);
	...
	bool Deploy()
	{
		return self.DefaultDeploy( self.GetV_Model( g_VeeMdl ), self.GetP_Model( g_PeeMdl ), CROWBAR_DRAW, "crowbar" );
	}
	...
	// end aone
                ...
                if (pEntity.IsBreakable())
                    pEntity.Use(m_pPlayer, self, USE_TOGGLE, 0.f);
			}
...
void Register()
{
    if (g_eModelsMode == kBlueShift) {
        g_VeeMdl = "models/scanarchy/bshift/v_crowbar.mdl";
    }
    
	g_CustomEntityFuncs.RegisterCustomEntity( "CLCROWBAR::weapon_clcrowbar", g_WeaponName );
	g_ItemRegistry.RegisterWeapon( g_WeaponName, "scanarchy" );
}
