...
// Hornet projectile
#include "sca_playerhornet"

namespace HLCHornetGun
{

const string g_WeaponName	= "weapon_hldm_hornetgun";
...
// Models
string g_PeeMdl		=  "models/hlclassic/p_hgun.mdl";
string g_VeeMdl		=  "models/hlclassic/v_hgun.mdl";
string g_WeeMdl		=  "models/hlclassic/w_hgun.mdl";

// Sounds
const string g_Fire1Snd		= "agrunt/ag_fire1.wav";
const string g_Fire2Snd		= "agrunt/ag_fire2.wav"; //ag_fire2.wav
const string g_Fire3Snd		= "agrunt/ag_fire3.wav"; //ag_fire3.wav

// Weapon Info
const uint g_DefGivePri		= 100;
const uint g_MaxAmmoPri		= 100;
...
// Fire rate delay
const float g_FRDelay = 0.25f;
const float g_RLDelay = 0.5f;

class CHLCHLDMHornetGun : ScriptBasePlayerWeaponEntity
{
	int m_iFirePhase;
	...
	void Precache()
	{
		...
		g_Game.PrecacheOther( "sca_playerhornet" );
	}
	...
	bool Deploy()
	{
		return self.DefaultDeploy( self.GetV_Model(g_VeeMdl), self.GetP_Model(g_PeeMdl), HGUN_UP, "hive" );
	}
	...
	void PrimaryAttack()
	{
		...
		// Shoot the hornet
		CBaseEntity@ pHornet = g_EntityFuncs.Create( "sca_playerhornet", m_pPlayer.GetGunPosition() + g_Engine.v_forward * 16 + g_Engine.v_right * 8 + g_Engine.v_up * -12, m_pPlayer.pev.v_angle, false, m_pPlayer.edict() );
	...
	void SecondaryAttack()
	{
		...
		CBaseEntity@ cbeclhornet = g_EntityFuncs.Create( "sca_playerhornet", vecSrc, m_pPlayer.pev.v_angle, false, m_pPlayer.edict() );
		SCAHORNETPROJECTILE::CPlayerHornet@ pHornet = cast<SCAHORNETPROJECTILE::CPlayerHornet@>( CastToScriptClass( cbeclhornet ) );
...
void Register()
{
	SCAHORNETPROJECTILE::RegisterHornetProjectile();
	g_CustomEntityFuncs.RegisterCustomEntity( "HLCHornetGun::CHLCHLDMHornetGun", HLCHornetGun::g_WeaponName );
	g_ItemRegistry.RegisterWeapon( HLCHornetGun::g_WeaponName, "scanarchy", "Hornets" );
}
