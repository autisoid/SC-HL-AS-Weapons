...
class weapon_ofm249 : ScriptBasePlayerWeaponEntity
{
  ...
  {
    Precache();
    g_EntityFuncs.SetModel(self, self.GetW_Model("models/scanarchy/w_saw.mdl"));
    self.m_iDefaultAmmo = DEFAULT_GIVE;
    self.FallInit();

    m_bAlternatingEject = false;
    @m_pDamage = g_EngineFuncs.CVarGetPointer("sk_556_bullet");
  }

  void Precache()
  {
    self.PrecacheCustomModels();
    g_Game.PrecacheModel("models/scanarchy/w_saw.mdl");
    g_Game.PrecacheModel("models/scanarchy/v_saw.mdl");
    g_Game.PrecacheModel("models/scanarchy/p_saw.mdl");

    m_iLink = g_Game.PrecacheModel("models/saw_link.mdl");
    m_iShell = g_Game.PrecacheModel("models/scanarchy/saw_shell.mdl");

    g_SoundSystem.PrecacheSound("weapons/saw_fire1.wav");
    g_SoundSystem.PrecacheSound("hlclassic/weapons/357_cock1.wav");
    g_SoundSystem.PrecacheSound("scanarchy/weapons/saw_reload.wav"); // sequence: 2; frame: 1; event 5004
    g_Game.PrecacheGeneric("sound/scanarchy/weapons/saw_reload.wav");
    g_SoundSystem.PrecacheSound("scanarchy/weapons/saw_reload2.wav"); // sequence: 3; frame: 0; event 5004
    g_Game.PrecacheGeneric("sound/scanarchy/weapons/saw_reload2.wav");
  ...
  bool Deploy()
  {
    self.DefaultDeploy(self.GetV_Model("models/scanarchy/v_saw.mdl"), self.GetP_Model("models/scanarchy/p_saw.mdl"), DRAW, "saw", 0, pev.body);
  ...
  void PrimaryAttack()
  {
    ...
    self.FireBullets(1, vecSrc, vecAiming, vecSpread, 8192.0f, BULLET_PLAYER_CUSTOMDAMAGE, 2, int(m_pDamage.value), m_pPlayer.pev);
...
void Register()
{
  if (!g_CustomEntityFuncs.IsCustomEntity(GetName()))
  {
    ...
    g_ItemRegistry.RegisterWeapon(GetName(), "scanarchy", "556", "", "ammo_556", "");
  }
}
